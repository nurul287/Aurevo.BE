...
- RAG chatbot improvements — remaining sessions of the improvement roadmap (see [`09-ai-chatbot-rag.md`](09-ai-chatbot-rag.md) "Planned Improvements"). Sessions A (eval harness), B (hybrid search), and C (re-ranking) are **built** — hybrid + `hybrid+rerank` are opt-in via `retrieve()`'s `opts.mode`, with vector still the eval-gated default (the rerank eval/default-flip is blocked on upgrading the free-tier Voyage key). The RAG monitoring dashboard's **backend** is also built (Session E — `chat_metrics` capture + `GET /admin/ai-metrics` + cleanup retention). Still open: **eval-driven retrieval tuning** (chunk-text cleanup + prompt tuning + embedding-model A/B; deferred — the eval is saturated at the current KB size and blocked on the free-tier Voyage key) and the **RAG dashboard frontend** (Session F — `/admin/ai` admin page with recharts consuming `/admin/ai-metrics`)
