...
- RAG chatbot improvements — remaining sessions of the improvement roadmap (see [`09-ai-chatbot-rag.md`](09-ai-chatbot-rag.md) "Planned Improvements"). Sessions A (eval harness), B (hybrid search), and C (re-ranking) are **built** — hybrid + `hybrid+rerank` are opt-in via `retrieve()`'s `opts.mode`, with vector still the eval-gated default (the rerank eval/default-flip is blocked on upgrading the free-tier Voyage key). Still open: **eval-driven retrieval tuning** (chunk-text cleanup + prompt tuning + embedding-model A/B) and the **RAG monitoring dashboard** (BE metrics capture + `/admin/ai-metrics` endpoint, FE admin page with recharts)
