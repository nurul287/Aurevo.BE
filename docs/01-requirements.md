...
- RAG chatbot improvements — remaining sessions of the improvement roadmap (see [`09-ai-chatbot-rag.md`](09-ai-chatbot-rag.md) "Planned Improvements"). Sessions A (eval harness) and B (hybrid search) are **done**; still open: **re-ranking** (Voyage `rerank-2.5-lite` on the hybrid candidate pool — the piece expected to make hybrid beat vector), **eval-driven retrieval tuning** (chunk-text cleanup + prompt tuning + embedding-model A/B), and the **RAG monitoring dashboard** (BE metrics capture + `/admin/ai-metrics` endpoint, FE admin page with recharts)
