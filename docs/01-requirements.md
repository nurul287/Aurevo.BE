...
- RAG chatbot improvements — **the improvement roadmap is complete** (see [`09-ai-chatbot-rag.md`](09-ai-chatbot-rag.md) "Planned Improvements"): retrieval evaluation harness, hybrid search, and re-ranking (Voyage `rerank-2.5-lite`, measured strictly ≥ vector at MRR 1.000 and **now the production default** via `retrieve()`'s `opts.mode`), the monitoring dashboard (backend `chat_metrics` + `GET /admin/ai-metrics` + retention; frontend `/admin/ai` recharts page), and an LLM-as-judge answer-quality eval (`pnpm eval:answers`). The one remaining sub-item, eval-driven **retrieval** tuning, is moot at the current KB size — reranking took retrieval to MRR 1.000, so the retrieval eval is saturated with no headroom to demonstrate a gain; revisit only if the catalog grows. Answer/prompt tuning is now measurable via the answer eval whenever desired
