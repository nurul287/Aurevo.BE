...
- Runs only against a **local** `DATABASE_URL` (refuses otherwise — it spends real Voyage quota and needs a seeded, `ingest:knowledge`-ed DB), and rides out free-tier 429s between queries. Deliberately a plain tsx script, never part of vitest/CI.
- `--mode` selects the retrieval strategy so modes are compared on the same dataset (`vector`, `hybrid`, `hybrid+rerank`). `--k` (default 3, matching the production `topK`), `--pace-ms` (sleep between queries to fit a rate-limited key's budget — a free-tier Voyage key is 3 RPM, and `hybrid+rerank` spends 2 calls/query), and `--json` for machine-readable output.
...
**Why hybrid isn't the default yet:** hybrid retrieval (FTS keyword leg + RRF fusion, `opts.mode: "hybrid"` on `retrieve()`) exists and is unit-proven to rescue lexical-only matches that vector search ranks last — but the eval gate showed it *marginally regressing* on this dataset. The entire diff is one query: "what is your return policy" AND-matches common terms across many chunks, and `ts_rank` boosts lexically-dense-but-irrelevant ones (exchange/sizing cross-references) into the fused top-3, evicting a relevant chunk. Meanwhile the messy-exact-title lookups hybrid targets are already rank-1 on vector alone at this KB size, so there was no headroom to win. The default flips when either (a) the reranker demonstrably demotes the fusion pollution, or (b) the KB grows enough that vector search starts missing exact lookups.

**Re-ranking (`hybrid+rerank`) — built, default-flip blocked on Voyage quota:** `retrieve()`'s `hybrid+rerank` mode reranks the fused candidate pool with Voyage `rerank-2.5-lite` (2500ms timeout, graceful fallback to fusion order on any failure — see below), and is unit-proven to apply the reranker's order and to fall back cleanly when it 429s. Its full 32-query eval could **not** be completed on the current free-tier Voyage key: `hybrid+rerank` spends two API calls per query (embed + rerank) against a shared 3 RPM bucket, and even with `--pace-ms 45000` and 12 retries the bucket wouldn't refill fast enough to finish. So the number that would justify a default flip doesn't exist yet, and `vector` stays the default. This is not a "rerank is worse" verdict — it's "not yet measured," gated on the same Voyage-plan upgrade already on the [Rollout checklist](#rollout-checklist). Structural caveat worth stating: at 31 chunks vector recall@3 is already 0.990 / hit-rate 1.000, so reranking has almost no headroom to win on *this* KB regardless — the rerank pass earns its keep as the KB grows and the candidate pool actually needs disambiguating.
...
- [ ] Confirm Voyage AI's production rate limits/quota are sufficient for expected traffic — **not yet done**: the production key hit the same free-tier 3 RPM/10K TPM limit during the initial backfill (no payment method added yet), same as the local dev key. Also blocks the `hybrid+rerank` retrieval eval (2 calls/query can't fit a 3 RPM bucket) and thus the reranker's default-flip decision — upgrading the Voyage plan unblocks both.
