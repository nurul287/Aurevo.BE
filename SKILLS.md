...
├── db/
│   ├── schema.ts                   ← HAND-AUTHORED source of truth (never introspect)
...
**Do not introspect.** `src/db/schema.ts` is hand-authored; `drizzle-kit introspect` would overwrite it and drop the RLS predicates (it emitted 34 of 49 policies with no `USING` clause). To change the schema: edit `schema.ts`, then `pnpm db:generate`, then `pnpm db:migrate`. See the "Database ownership" section of [`CLAUDE.md`](CLAUDE.md).
