...
**Bulk product import** (`src/app/modules/imports/`, `src/workers/import.worker.ts`) — full architecture, worker semantics, and verification log in [`docs/10-bulk-import-pipeline.md`](docs/10-bulk-import-pipeline.md). Spreadsheet/JSON/NDJSON upload → `import_jobs`/`import_rows` (migration 045) → BullMQ (Redis) → a separate worker process (`pnpm worker`, not part of the API server). The one internal contract every source converges on is `NormalizedProduct` (`imports.schema.ts`) — the spreadsheet parser and the standalone scraper package (`Aurevo/scraper`) both produce it, so the pipeline itself is source-agnostic.
- **Hook-free bulk insert**: `products.service.ts`'s `insertProduct` is the raw-insert core extracted from `createProduct`, deliberately skipping the per-product Voyage embed hook that the normal admin-create path fires — the worker batch-embeds every product touched by a job in one pass at the end instead (`knowledge.service.ts`'s `batchUpsertProductChunks`), turning what would be 1000 sequential embed calls into a handful of batched ones.
- **Idempotent re-import**: `products.external_id`/`source` (unique together) means re-uploading or re-scraping the same source row updates the existing product instead of duplicating it. Row-level failures (most commonly a same-batch slug collision under the worker's row concurrency) don't kill the job — `POST /admin/imports/:jobId/retry` re-enqueues only the rows still `failed`.
- **Known gaps** (see the doc's Backlog): `Content-Disposition` isn't in the CORS `Access-Control-Expose-Headers` list, so admin file downloads save with a generic filename; a Redis outage during upload can leave a job stuck at `pending` with rows already committed but never enqueued.
