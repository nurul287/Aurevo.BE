...
**AI chat / RAG** (`src/app/modules/chat/`, `src/app/modules/knowledge/`) — full architecture, diagram, and rollout checklist in [`docs/09-ai-chatbot-rag.md`](docs/09-ai-chatbot-rag.md). Customer-facing only, no admin mode. `knowledge.service.ts` embeds product/policy/FAQ text via Voyage AI (`src/lib/voyage.ts`) into `kb_chunks` (pgvector, cosine distance). `chat.service.ts` streams real Anthropic tokens (`stream: true`, not simulated) through a tool-use loop with three tools: `search_knowledge` (semantic, optional `sourceType` filter, `topK = 3`), `get_product_details` (live stock/price by slug, resolves multiple slugs concurrently), and `get_my_orders` — **only ever added to the tool list when the request is authenticated**, and its query is hard-scoped to `req.user.id` server-side regardless of tool input, so neither a guest session nor a prompt-injection attempt has any path to another customer's orders.
